...
echo -n "작성자 이름 입력(ex: Lee,SungJin): "
read NAME
...
do
	...
	GW=$(nmcli device show $NIC | grep 'IP4.ROUTE' | grep 'dst = 0.0.0.0/0' | awk '{print $7}' | awk -F, '{print$1}')
	DNSLINE=$(nmcli device show $NIC | grep 'IP4.DNS' | awk '{print $2}')
	DNS=$(echo $DNSLINE | tr '\n' ' ')
	cat << EOF
Network : $NIC
	IP  : $IP
